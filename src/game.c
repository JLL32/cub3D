...
static t_data	load_texture(char *path, t_game *game)
{
	...
	texture_buffer.img = mlx_xpm_file_to_image(
		game->mlx, path, &texture_buffer.width, &texture_buffer.height);
	free(path);
...
t_game			create_game(int argc, char **argv)
{
	...
	game.player =
		create_player(cfg.player.pos_x, cfg.player.pos_y, cfg.player.dir);
	game.colors = cfg.colors;
	game.res = cfg.res;
	game.world_map = cfg.map;
	ft_memcpy(game.sprite_positions, cfg.sprites,
			cfg.sprite_count * sizeof(t_coordinate));
	game.sprite_count = cfg.sprite_count;
	game.is_save = cfg.is_save;
	game.mlx = mlx_init();
	game.win = mlx_new_window(game.mlx, game.res.width, game.res.height,
		"Cub3D");
	game.win_buffer.img =
		mlx_new_image(game.mlx, game.res.width, game.res.height);
	game.win_buffer.addr =
		(int *)mlx_get_data_addr(game.win_buffer.img,
								&game.win_buffer.bits_per_pixel,
								&game.win_buffer.line_length,
								&game.win_buffer.endian);
